...
import com.lathx.springcloud.lb.LoadBalancer;
...
public class OrderController {
    ...
    @GetMapping("/consumer/payment/getPayment/lb/{id}")
    public CommonResult<Payment> getPaymentBySelfLoadBalance(@PathVariable("id") Long id) {

        // 获得所有 CLOUD-PAYMENT-SERVICE 服务
        List<ServiceInstance> instances = discoveryClient.getInstances("CLOUD-PAYMENT-SERVICE");
        if (instances == null || instances.size() == 0) {
            return null;
        }
        // 获得所有 CLOUD-PAYMENT-SERVICE 服务的URI
